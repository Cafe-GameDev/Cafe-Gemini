# Script para instalar, atualizar ou desinstalar a personalidade "cafe-gemini".
...
# --- Logica de Desinstalacao ---
if ($Uninstall) {
    Write-Host "----------------------------------------------------------------"
    Write-Host "Iniciando a desinstalacao do assistente 'cafe-gemini'…"
    Write-Host "----------------------------------------------------------------"

    # 1. Remover funcoes do perfil do PowerShell
    if (Test-Path $PROFILE) {
        Write-Host "-> Removendo configuracoes do perfil do PowerShell..."
        $CurrentProfileContent = Get-Content $PROFILE -Raw -ErrorAction SilentlyContinue
        # O padrao regex localiza o bloco de comentario e tudo ate o proximo bloco ou o fim do arquivo
        $pattern = "(?s)" + ($([regex]::Escape($Comment))) + ".*?(?=^# Comandos|\z)"
        if ($CurrentProfileContent -match $pattern) {
            $NewContent = $CurrentProfileContent -replace $pattern, ''
            Set-Content -Path $PROFILE -Value $NewContent.Trim()
            Write-Host "Configuracoes removidas do perfil."
        } else {
            Write-Host "Configuracoes nao encontradas no perfil."
        }
    } else {
        Write-Host "AVISO: Perfil do PowerShell nao encontrado. Pulei a limpeza."
    }

    # 2. Remover o diretorio de instalacao
    if (Test-Path $InstallDir) {
        Write-Host "-> Removendo o diretorio de instalacao '$InstallDir'…"
        Remove-Item -Recurse -Force -Path $InstallDir
        Write-Host "Diretorio removido."
    } else {
        Write-Host "-> Diretorio de instalacao nao encontrado."
    }

    Write-Host ""
    Write-Host "----------------------------------------------------------------"
    Write-Host "✅ Desinstalacao concluida."
    Write-Host "Para finalizar, reinicie seu terminal."
    Write-Host "----------------------------------------------------------------"
    exit 0
}
...
Write-Host "  powershell.exe -File '$InstallDir\install.ps1' -Uninstall - Desinstala o assistente."
Write-Host "----------------------------------------------------------------"
