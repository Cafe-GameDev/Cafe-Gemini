...
uninstall() {
    echo "----------------------------------------------------------------"
    echo "Iniciando a desinstalacao do assistente 'cafe-gemini'…"
    echo "----------------------------------------------------------------"

    # 1. Detectar o arquivo de configuracao do shell
    SHELL_CONFIG_FILE=""
    ...
    if [ -n "$SHELL_CONFIG_FILE" ] && [ -f "$SHELL_CONFIG_FILE" ]; then
        echo "-> Removendo configuracoes de '$SHELL_CONFIG_FILE'…"
        # Usar sed para remover as linhas do alias e o comentario
        sed -i.bak -e "/$COMMENT/d" \
                   -e "/alias $PERSONALITY_NAME=/d" \
                   -e "/alias $UPDATE_CMD_NAME=/d" "$SHELL_CONFIG_FILE"
        echo "Configuracoes removidas. Um backup foi criado em '$SHELL_CONFIG_FILE.bak'."
    else
        echo "AVISO: Arquivo de configuracao do shell nao encontrado. Pulei a limpeza dos aliases."
    fi

    # 2. Remover o diretorio de instalacao
    if [ -d "$INSTALL_DIR" ]; then
        echo "-> Removendo o diretorio de instalacao '$INSTALL_DIR'…"
        rm -rf "$INSTALL_DIR"
    ...
    echo "Para finalizar, reinicie seu terminal."
    echo "----------------------------------------------------------------"
    exit 0
}
...
# --- Inicio do Script de Instalacao/Atualizacao ---
echo "----------------------------------------------------------------"
...
echo "  bash \"$INSTALL_DIR/install.sh\" --uninstall - Desinstala o assistente."
echo "----------------------------------------------------------------"
